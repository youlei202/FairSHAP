...
def marginalised_contingency(y, hx, vY, dY):
  ...
  for i in range(dY):
    for j in range(dY):
      tmp = np.logical_and(np.equal(y, vY[i]),
                           np.equal(hx, vY[j]))
...
def marginalised_confusion(Cij, loc=1):
  ...
  Cm[1, 1] = (np.sum(Cij) + Cij[loc, loc] - np.sum(Cij[loc])
              - np.sum(Cij[:, loc]))
...
def marginalised_matrix(y, hx, pos=1, priv=1, sen=list()):
  ...
  dY = len(vY)
  gones_y_, gzero_y_, gones_hx, gzero_hx = marginalised_split_up(
      y, hx, priv, sen)
...
def perturb_pandas_ver(X, sen_att, priv_val, ratio=.5):
  ...
  for i, ti in enumerate(X.index):
    ...
    for j, sa, pv, un in zip(range(dim),
                             sen_att, priv_val, unpriv_dict):
      if not prng[j]:
        continue
