...
from src.attribution.oracle_metric import (
    marginalised_contingency,
...
from src.attribution.oracle_metric import perturb_pandas_ver, perturb_numpy_ver
from src.attribution.oracle_metric import (
    ell_fair_x,
    ell_loss_x,
    hat_L_fair,
...
def marginal_bin(n, nc):
    ...
    g1_y, g0_y, g1_hx, g0_hx = marginalised_split_up(y, hx, priv=1, sen=sen)
    assert len(g1_y) + len(g0_y) == n
    assert len(g1_hx) + len(g0_hx) == n
    assert sum(g1_y) + sum(g0_y) == sum(y)
    assert sum(g1_hx) + sum(g0_hx) == sum(hx)

    g1_Cij, g0_Cij, g1_Cm, g0_Cm = marginalised_matrix(y, hx, pos=1, priv=1, sen=sen)
    h1_Cij, h0_Cij, h1_Cm, h0_Cm = marginalised_pd_mat(y, hx, pos=1, idx_priv=sen == 1)
    """
...
def fair_bound(n, nd):
    ...
    sen_att = [1, 3]
    X[:, 1] = (X[:, 1] > 0.35).astype("int")
    # X[:, 3] = (X[:, 3] < .7).astype('int')
    X[:, 3] = np.random.randint(3, size=n)

    X_prime = pd.DataFrame(X, columns=["height", "sex", "weight", "race"])
    sen_att_prime = ["sex", "race"]
    priv_val = [1, 1]
    ratio = 0.7

    X_pertb = perturb_pandas_ver(X_prime, sen_att_prime, priv_val, ratio)
    X_qtb = perturb_numpy_ver(X, sen_att, priv_val, ratio)

    tmp = (
        X_pertb.drop(columns=sen_att_prime) == X_prime.drop(columns=sen_att_prime)
    ).all()
