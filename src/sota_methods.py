...
class SOTAPreprocessingMethods:
        ...
        for i in ['german_credit', 'compas', 'compas4race', 'adult', 'default_credit', 'census_income_kdd']:
            ...
            for j, (train_index, val_index) in enumerate(kf.split(processed_data)):
                ...
                if self.sota_method == 'disparate_impact':
                    X_train_repair = self._disparate_impact_remover(repair_level=1.0, sensitive_attribute=self.sen_att_name, X=X_train)
                    X_val_repair = self._disparate_impact_remover(repair_level=1.0, sensitive_attribute=self.sen_att_name, X=X_val)
                    model = XGBClassifier()
    ...
    def _disparate_impact_remover(self, repair_level, sensitive_attribute, X):
        DisparateImpactRemover(repair_level=1.0, sensitive_attribute=self.sen_att_name)
        from BlackBoxAuditing.repairers.GeneralRepairer import Repairer
        self.Repairer = Repairer
        features = X.values.tolist()
        index = X.columns.tolist().index(self.sen_att_name)
        repairer = self.Repairer(features, index, repair_level, False)

        repaired_features = repairer.repair(features)
        repaired_features = np.array(repaired_features, dtype=np.float64)
